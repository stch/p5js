...
ColorCluster.prototype.addColor = function(colstr){
  //colors[colstr] = (colors[colstr] + 1) || 1
...
function gotFile(file) {
    ...
    if (file.type === 'image') {
        ...
        for (let i = 0; i < img_p.pixels.length; i += 4) {
            ...
            if (!targetCluster.includes(color2str([img_p.pixels[i], img_p.pixels[i + 1], img_p.pixels[i + 2]])])) {
                img_p.pixels[i + 3] = 0
            }
        }
        img_p.updatePixels()
...
function clusterize_devider(cluster) {
    ...
    cluster.forEach(function(colst) {
        ...
        divider[colst_div].set(colst,cluster.getColor(colst))
...
//            if (cluster.hasOwnProperty(cl)) {
        cluster.forEach(function(){
                stroke("#"+colst.slice(0,6))
...
function drawAccumH(dw) {
    ...
    for (let l = 0; l < dwt; l++) {
        ...
        for (let i = l * pixel_width * 4; i < (l * pixel_width + dwt) * 4; i += 4) {
            pixcount++
            if (pixels[i + 3] != 0) {
                var colst = color2str([pixels[i], pixels[i + 1], pixels[i + 2], pixels[i + 3]])
                accum[l][colst] = accum[l][colst] + 1 || 1
            } else {
                // alpha==0のものは色計算に影響を与えないように別集計
                accum[l]["00000000"]++
            }
        }
    }
